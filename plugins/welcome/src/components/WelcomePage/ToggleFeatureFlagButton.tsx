...
const ToggleFeatureFlagButton: FC<{}> = () => {
  ...
  const handleClick = () => {
    const state = featureFlagsApi.get('enable-welcome-box');

    if (state) {
      featureFlagsApi.set('enable-welcome-box', FeatureFlagState.NotEnabled);
    } else {
      featureFlagsApi.set('enable-welcome-box', FeatureFlagState.Enabled);
    }
